...
public class JurosSimplesServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>*-*-*-*-*-*-*-*-Calculo Juros Simples-*-*-*-*-*-*-*-*</h1>");
            
           double vp = Double.parseDouble(request.getParameter("vp"));
           double j = Double.parseDouble(request.getParameter("j")) / 100;
           double n = Double.parseDouble(request.getParameter("n"));
           double vf = vp*(1 +j * n);;
           out.println("<br>");
           out.println("Formula: VF = VP (1 + j * n)");
           out.println("<br><br>");
           out.println("<h3>Resultado: " + vf + "</h3>");
